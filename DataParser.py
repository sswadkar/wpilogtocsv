...
__all__ = ["StartRecordData", "MetadataRecordData",
           "DataLogRecord", "DataLogReader"]
...
class DataLogRecord:
    ...
    def getStartData(self) -> StartRecordData:
        ...
        entry = int.from_bytes(
            self.data[1:5], byteorder="little", signed=False)
    ...
    def _readInnerString(self, pos: int):
        size = int.from_bytes(
...
class DataLogIterator:
    ...
    def __next__(self) -> DataLogRecord:
        ...
        timestamp = self._readVarInt(
            self.pos + 1 + entryLen + sizeLen, timestampLen)
...
class DataParser():
    """
    Parses data from WPIlog format to a Pandas Dataframe.
            ...
            with open(path, "r") as f:
                with open(f"{csv_name}.csv", 'w', newline='') as csvfile:
                    ...
                    for record in reader:
                        ...
                        if found_keys and not (added_keys_to_csv):
                            csvfile.write(",".join(log_keys).strip() + "\n")
                            added_keys_to_csv = True
                        ...
                        if record.isStart():
                            ...
                            if not (found_keys) and log_keys != [entry.name for entry in list(entries.values())]:
                                log_keys = [
                                    entry.name for entry in list(entries.values())]
                            else:
                                found_keys = True
                            ...
                            if (found_keys):
                                ...
                                if ("Console" not in entry.name):
                                    # iteratively check whether all the log keys have been obtained by comparing to previous iteration

                                    # print(
                                    #     f"Data({record.entry}, size={len(record.data)}) ", end="")
                                    ...
                                    try:
                                        ...
                                        if entry.name == "systemTime" and entry.type == "int64":
                                            dt = datetime.fromtimestamp(
                                                record.getInteger() / 1000000)
                                            data_entry = "  {:%Y-%m-%d %H:%M:%S.%f}".format(
                                                dt)
                                            continue

                                        if entry.type == "double":
                                            data_entry = f"{record.getDouble()}"
                                        elif entry.type == "int64":
                                            data_entry = f"{record.getInteger()}"
                                        elif entry.type in ("string", "json"):
                                            data_entry = f"'{record.getString()}'"
                                        elif entry.type == "msgpack":
                                            data_entry = f"'{record.getMsgPack()}'"
                                        elif entry.type == "boolean":
                                            data_entry = f"{record.getBoolean()}"
                                        elif entry.type == "boolean[]":
                                    ...
                                    if (entry.name == "/Timestamp"):
                                        csvfile.write(
                                            ",".join(timestamp_data) + "\n")
                                        timestamp_data = []
                                    timestamp_data.append(data_entry)
